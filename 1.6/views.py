...
from models import User, Session, Token, Advertisement
...
def create_user():
    ...
    user_data['password'] = bcrypt.generate_password_hash(user_data.pop('password').encode()).decode()
...
def delete_token():
    ...
    with Session() as session:
        user = password_auth(session, user_data)
        try:
            token = session.query(Token).filter(Token.id == user_data['token']).first()
...
class AdvertisementView(MethodView):
    ...
    def get(self, adv_id: int):
        with Session() as session:
            adv = session.query(Advertisement).filter(Advertisement.id == adv_id).join(Advertisement.user).first()
            if not adv:
